...
def find_cheapest_product(client: TLSClient, shop_url: str, min_price: float = 0.50,
    ...
    - Retries up to 2 times on 429/400/5xx or any network exception (reduced from 3)
    ...
    for attempt in range(1, 3):  # 2 attempts (was 3)
        try:
            resp = client.get(url)
        except Exception as exc:
            last_err = exc
            logger.warning(
                "find_cheapest_product attempt %d/2 network error: %s", attempt, exc
            )
            if attempt < 2:
        ...
        if resp.status_code in _RETRYABLE:
            ...
            logger.warning(
                "find_cheapest_product attempt %d/2: HTTP %s", attempt, resp.status_code
            )
            if attempt < 2:
                time.sleep(2 + random.random() * 2)
            continue
...
def run_checkout_for_card(shop_url: str, card_entry: str, proxy_url: str = "", low: bool = True) -> CheckResult:
    ...
    try:
        ...
        try:
            ...
            MAX_TAX_RETRIES = 2  # reduced from 3
        ...
        for poll_num in range(1, 11):  # reduced from 21 to 11
            try:
