...
async def perform_checkout(site, card, month, year, cvv):
    try:
        ...
        return {"Response": f"Playwright error: {str(e)[:100]}", "Price": 0, "Gateway": "Shopify"}
...
def shopify():
    try:
        ...
        if not site or not cc:
            return jsonify({"Response": "Missing site/cc", "Price": 0, "Gateway": "Shopify"})
        card, month, year, cvv = extract_cc(cc)
        if not card:
            return jsonify({"Response": "Invalid card format", "Price": 0, "Gateway": "Shopify"})
        if not site.startswith('http'):
            site = 'https://' + site
        result = asyncio.run(perform_checkout(site, card, month, year, cvv))
        return jsonify(result)
    except Exception as e:
        return jsonify({"Response": f"Server error: {str(e)[:100]}", "Price": 0, "Gateway": "Shopify"})
