...
# Expanded with all required fields – every country now has complete data.
...
def address_for_country(country: str) -> Address:
    """Return a fully filled address, using fallback if country is not in the table."""
    base = country[:2].upper()
    addr = COUNTRY_ADDRESSES.get(base)
    if addr is None:
        # Build a generic address with the given country code
        # Use a simple mapping for common zones/postal codes
        zone_map = {
            "US": "NY", "CA": "ON", "GB": "ENG", "AU": "NSW", "DE": "BE", "FR": "IDF",
            "NL": "NH", "IE": "D", "NZ": "AUK", "ES": "M", "IT": "RM", "SE": "AB",
            "NO": "03", "DK": "84", "FI": "18", "BE": "BRU", "CH": "ZH", "AT": "9",
            "JP": "13", "SG": "01", "AE": "DU"
        }
        postal_map = {
            "US": "10036", "CA": "K1A 0G9", "GB": "SW1A 2AA", "AU": "2000", "DE": "10117",
            "FR": "75001", "NL": "1012 JS", "IE": "D02 Y006", "NZ": "1010", "ES": "28013",
            "IT": "00184", "SE": "111 20", "NO": "0154", "DK": "1457", "FI": "00100",
            "BE": "1000", "CH": "8001", "AT": "1010", "JP": "100-0005", "SG": "048616",
            "AE": "12345"
        }
        city_map = {
            "US": "New York", "CA": "Ottawa", "GB": "London", "AU": "Sydney", "DE": "Berlin",
            "FR": "Paris", "NL": "Amsterdam", "IE": "Dublin", "NZ": "Auckland", "ES": "Madrid",
            "IT": "Rome", "SE": "Stockholm", "NO": "Oslo", "DK": "Copenhagen", "FI": "Helsinki",
            "BE": "Brussels", "CH": "Zurich", "AT": "Vienna", "JP": "Tokyo", "SG": "Singapore",
            "AE": "Dubai"
        }
        zone = zone_map.get(base, "NY")
        postal = postal_map.get(base, "12345")
        city = city_map.get(base, "Anytown")
        addr = Address(
            first_name="james",
            last_name="anderson",
    ...
    # Ensure every field is non-empty (fill with dummy if missing)
    return Address(
        first_name=addr.first_name or "james",
        last_name=addr.last_name or "anderson",
        address1=addr.address1 or "123 Main St",
        address2=addr.address2 or "",
        city=addr.city or "Anytown",
        country_code=addr.country_code or base,
        zone_code=addr.zone_code or "NY",
        postal_code=addr.postal_code or "12345",
        phone=addr.phone or "+12125550100"
    )
...
def find_cheapest_product(client: TLSClient, shop_url: str, min_price: float = 0.50,
    ...
    - Retries up to 3 attempts with exponential backoff on 429 and network errors.
    ...
    for attempt in range(1, 4):  # increased to 3 attempts
        try:
            ...
            last_err = exc
            wait = (2 ** attempt) + random.random()  # 2,4,8 seconds
            logger.warning(
                "attempt %d/3 network error: %s, retry in %.1fs", attempt, exc, wait
            )
            time.sleep(wait)
            continue
        ...
        if resp.status_code == 429:
            wait = (2 ** attempt) * 5 + random.random()  # 10,20,40 seconds
            logger.warning("HTTP 429, retry in %.1fs", wait)
            time.sleep(wait)
            continue
